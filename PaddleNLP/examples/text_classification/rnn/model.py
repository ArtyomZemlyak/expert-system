...
class LSTMModel(nn.Layer):
    ...
    def forward(self, text, seq_len):
        ...
        embedded_text = self.dropout1(embedded_text)
...
class LSTMGRUModel(nn.Layer):
    def __init__(self,
                 ...
                 lstm_direction='bidirect',
                 lstm_layers=1,
                 gru_hidden_size=198,
                 gru_direction='bidirect',
                 gru_layers=1,
                 dropout_rate=0.0,
                 pooling_type=None,
                 fc_hidden_size=96):
    ...
    def forward(self, text, seq_len):
        ...
        # if not, num_directions = 1
        text_repr = self.lstm_encoder(embedded_text, sequence_length=seq_len)
        text_repr = self.gru_encoder(text_repr, sequence_length=seq_len)
        # Shape: (batch_size, fc_hidden_size)
        fc_out = paddle.tanh(self.fc(text_repr))
        # Shape: (batch_size, num_classes)
        logits = self.output_layer(fc_out)
...
class CNNModel(nn.Layer):
    ...
    def __init__(self,
        ...
        self.fc = nn.Linear(160, fc_hidden_size)
        self.output_layer = nn.Linear(fc_hidden_size, num_classes)
    ...
    def forward(self, text, seq_len=None):
        ...
        encoder_out = paddle.reshape(encoder_out, (len(text), 16, 16))
        max_pool_out = self.max_pool(encoder_out)
        avg_pool_out = self.avg_pool(encoder_out)
        encoder_out = paddle.concat([max_pool_out, avg_pool_out], axis=1)
        encoder_out = paddle.flatten(encoder_out, start_axis=1)
        #encoder_out = self.dropout2(encoder_out)
...
class TCNModel(nn.Layer):
    ...
    def __init__(self,
        ...
        self.embedder = nn.Embedding(
            vocab_size, emb_dim, padding_idx=padding_idx)
        self.encoder = nlp.seq2vec.TCNEncoder(
            input_size=emb_dim,
