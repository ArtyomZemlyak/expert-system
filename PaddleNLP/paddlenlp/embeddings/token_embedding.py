...
class TokenEmbedding(nn.Embedding):
    ...
    def __init__(self,
        ...
        vector_path = os.path.join(home_path, '.paddlenlp/models/embeddings/ft_native_300_ru_wiki_lenta_nltk_word_tokenize.vec')
        ...
        def load_vectors(fname):
            ...
            print(n, '|', d)
            LEN = 250000
            data = np.zeros((LEN, d), dtype=np.float32)
            w_i = {PAD_TOKEN: 0, UNK_TOKEN: 1}
            i_w = {0: PAD_TOKEN, 1: UNK_TOKEN}

            with Progress() as progress:
                task = progress.add_task("[green]Loading embedding vector ...", total=LEN)

                for i, line in enumerate(fin):
                    if i < LEN:
                        #logger.info(line)
                        tokens = line.rstrip().split(' ')
                        #logger.info(np.float32(tokens[1:]))
                        data[i] = np.float32(tokens[1:])# map(float, tokens[1:])
                        w_i[tokens[0]] = i
                        i_w[i] = [tokens[0]]
                    else: break
            ...
            unk_vector = np.random.normal(
                scale=0.02,
                size=d).astype(paddle.get_default_dtype())
            pad_vector = np.array(
                [0] * d).astype(paddle.get_default_dtype())

            embedding_table = np.append(
                [pad_vector, unk_vector], data, axis=0)

            return embedding_table, LEN+2, d, w_i, i_w

        embedding_table, self.num_embeddings, self.embedding_dim, self._word_to_idx, self._idx_to_word = load_vectors(vector_path) # np.load(vector_path)
        ...
        # import json
        # vocab_dict = json.loads(open(os.path.join(home_path, 'BI/expert-system/TEST/train_data_4c/vocab.json'), "r").read())['model']['vocab']
        # self.vocab = Vocab.from_dict(vocab_dict, unk_token='[UNK]', pad_token='[PAD]')
        ...
        super(TokenEmbedding, self).__init__(
            self.num_embeddings,
            self.embedding_dim,
            padding_idx=0)#self._word_to_idx[PAD_TOKEN])
