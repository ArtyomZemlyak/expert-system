...
def draw_networkx_labels(
    ...
    if type(font_size) != list:
        font_sizes = [font_size for i in range(len(labels.items()))]
    else:
        font_sizes = font_size
    if type(alpha) != list:
        alphas = [alpha for i in range(len(labels.items()))]
    else:
        alphas = alpha
    for (n, label), font_size, alpha in zip(labels.items(), font_sizes, alphas):
        (x, y) = pos[n]
...
def draw_networkx_edge_labels(
    ...
    if type(font_size) != list:
        ...
        font_sizes = font_size
    for ((n1, n2), label), font_size in zip(labels.items(), font_sizes):
        (x1, y1) = pos[n1]
        (x2, y2) = pos[n2]
        (x, y) = (
