...
class RASASkill(Component):
    ...
    def __call__(
        self, utterances_batch: List[str], states_batch: Optional[List] = None
        ...
        """
        user_ids, output_states_batch = self._handle_user_identification(
            utterances_batch, states_batch
        )
        #################################################################################
        ...
        futures = [
            self.rasa_confident_response_decorator(self._agent, utt, sender_id=uid)
            for utt, uid in zip(utterances_batch, user_ids)
        ]
    ...
    async def rasa_confident_response_decorator(
        self, rasa_agent, text_message, sender_id
    ):
        """
        Args:
        ...
        resp = await self.rasa_handle_text_verbosely(
            rasa_agent, text_message, sender_id
        )
        if resp:
    ...
    async def rasa_handle_text_verbosely(self, rasa_agent, text_message, sender_id):
        ...
        """
        message = UserMessage(text_message, output_channel=None, sender_id=sender_id)
        ...
        while (
            ...
            and processor._should_handle_message(tracker)
            and num_predicted_actions < processor.max_number_of_predictions
        ):
        ...
        if is_action_limit_reached():
            ...
            if processor.on_circuit_break:
                ...
                processor.on_circuit_break(
                    tracker, message.output_channel, processor.nlg
                )
