...
class DSLMeta(ABCMeta):
    ...
    def __handle_batch(
        cls: "DSLMeta", utterances_batch: List[str], user_ids_batch: List[UserId]
    ) -> Tuple[List, ...]:
        ...
        return (
            *map(
                list,
                zip(
                    *starmap(cls.handle, zip_longest(utterances_batch, user_ids_batch))
                ),
            ),
        )
    ...
    @staticmethod
    def __handle(cls: "DSLMeta", utterance: str, user_id: UserId) -> SkillResponse:
        """
    ...
    def __select_handler(cls, context: UserContext) -> Optional[Callable]:
        """
    ...
    def __run_handler(
        cls, handler: Optional[Callable], context: UserContext
    ) -> SkillResponse:
        """
