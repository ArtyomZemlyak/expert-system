...
class TrainSetGeneration:
    ...
    def __init__(
        ...
        self.ds_tracker = DialogueStateTracker(
            api_call_id=0,
            slot_names=list(self.slotfiller._slot_vals.keys()),
            ...
            set(
                list(
                    itertools.chain.from_iterable(
                        map(
                            lambda x: re.findall(r"#(\w+)", x.text),
    ...
    def get_id_input(self, prompt: str, valid_vals: List[int]) -> int:
        # for neat output
        print("\n" + "*" * 10)
        idx = -1
        while idx == -1:
    ...
    def get_user_input(self) -> None:
        ...
        has_slot = self.get_id_input(
            prompt="type 1 if your sentence has a slot, else 0: ", valid_vals=[0, 1]
        )
        slots = []
        if has_slot:
            ...
            while has_slot:
                for i, key in enumerate(self.slots):
                    print(i, key)
                idx = self.get_id_input(
                    prompt="type slot category number from the list: ",
                ...
                if slot_category in self.slotfiller._slot_vals:
                    ...
                    for i, key in enumerate(self.slotfiller._slot_vals[slot_category]):
                        ...
                        id2key[i] = key
                    idx = self.get_id_input(
                        prompt="type slot subcategory number from the list: ",
                ...
                has_slot = self.get_id_input(
                    prompt="type 1 if you want to add more slots, else 0: ",
                    valid_vals=[0, 1],
                )

        user_input = {"speaker": 1, "text": text, "slots": slots}
    ...
    def start_generation(self) -> None:
        ...
        while True:
            turn = self.get_id_input(
                prompt="choose turn (1 for user, 2 for bot, 3 to start a new dialog or 10 for saving and exit): ",
                valid_vals=[1, 2, 3, 10],
            )
            print("\n" + "*" * 10)
            if turn == 1:
    ...
    def get_bot_output(self) -> None:
        ...
        slots = [
            [slot, self.slots_history[slot]]
            for slot in template_slots
            if slot in self.slots_history and slot in self.slotfiller._slot_vals
        ]
