...
class MD_YAML_DialogsDatasetReader(DatasetReader):
    ...
    @overrides
    def read(
        cls, data_path: str, dialogs: bool = False, ignore_slots: bool = False
    ) -> Dict[str, List]:
        """
        Parameters:
        ...
        intent2slots2text, slot_name2text2value = cls._read_intent2text_mapping(
            Path(data_path, nlu_fname), domain_knowledge, ignore_slots
        )

        short2long_subsample_name = {"trn": "train", "val": "valid", "tst": "test"}
    ...
    @classmethod
    def _read_intent2text_mapping(
        cls,
        nlu_fpath: Path,
        domain_knowledge: DomainKnowledge,
        ignore_slots: bool = False,
    ) -> Tuple[
        Dict[str, Dict[SLOT2VALUE_PAIRS_TUPLE, List]], Dict[str, Dict[str, str]]
    ]:
        ...
        with open(nlu_fpath) as nlu_f:
            for line in nlu_f:
                ...
                if line.strip().startswith("-"):
                    ...
                    line_slots_found = re.finditer(
                        slots_markup_pattern, intent_text_w_markup
                    )
                    if ignore_slots:
                        line_slots_found = []
                    ...
                    for line_slot in line_slots_found:
                        ...
                        assert slot_name in domain_knowledge.known_slots, (
                            f"{slot_name} from {nlu_fpath}"
                        ...
                        slot_name2text2value[slot_name][slot_value_text].append(
                            slot_value
                        )
                    ...
                    slots_key = tuple(
                        sorted((slot[0], slot[1]) for slot in cleaned_text_slots)
                    )
    ...
    def _read_story(
        ...
        List[List[Tuple[Dict[str, bool], Dict[str, Any]]]],
        List[Tuple[Dict[str, bool], Dict[str, Any]]],
    ]:
        """
        ...
        """
        log.debug(
            f"BEFORE MLU_MD_DialogsDatasetReader._read_story(): "
        ...
        def process_user_utter(line: str) -> List[List[Dict[str, Any]]]:
            ...
            try:
                possible_user_utters = cls.augment_user_turn(
                    intent2slots2text, line, slot_name2text2value
                )
                ...
                log.debug(
                    f"INSIDE MLU_MD_DialogsDatasetReader._read_story(): "
        ...
        def process_system_utter(line: str) -> List[List[Dict[str, Any]]]:
            ...
            if system_action_name.startswith("form"):
                ...
                augmented_utters = cls.augment_form(
                    form_name, domain_knowledge, intent2slots2text
                )
        ...
        for line in story_file:
            ...
            if line.startswith("#"):
                ...
                if (
                    curr_story_utters_batch
                    and curr_story_utters_batch[0]
                    and curr_story_utters_batch[0][-1]["speaker"]
                    == cls._USER_SPEAKER_ID
                ):
                    for curr_story_utters in curr_story_utters_batch:
                        curr_story_utters.append(
                            default_system_goodbye
                        )  # dialogs MUST end with system replics
                ...
                if not nonlocal_curr_story_bad:
                    for curr_story_utters_ix, curr_story_utters in enumerate(
                        ...
                        stories_parsed[
                            curr_story_title + f"_{curr_story_utters_ix}"
                        ] = curr_story_utters
        ...
        gobot_formatted_stories = DSTC2DatasetReader._read_from_file(
            tmp_f.name, dialogs=dialogs
        )
        os.remove(tmp_f.name)

        log.debug(
            f"AFTER MLU_MD_DialogsDatasetReader._read_story(): "
            f"story_fpath={story_fpath}, "
            f"dialogs={dialogs}, "
            f"domain_knowledge={domain_knowledge}, "
            f"intent2slots2text={intent2slots2text}, "
    ...
    @classmethod
    def augment_form(
        cls, form_name: str, domain_knowledge: DomainKnowledge, intent2slots2text: Dict
    ) -> List[str]:
        """
        ...
        for slot_name, slot_info_li in form.items():
            if slot_info_li and slot_info_li[0].get("type", "") == "from_entity":
                ...
                augmended_story.extend(
                    cls.augment_slot(
                        known_responses, known_intents, slot_name, form_name
                    )
                )
    ...
    def augment_slot(
        ...
        """
        ask_slot_act_name = cls.get_augmented_ask_slot_utter(
            form_name, known_responses, slot_name
        )
        inform_slot_user_utter = cls.get_augmented_ask_intent_utter(
    ...
    @classmethod
    def get_augmented_ask_intent_utter(
        cls, known_intents: List[str], slot_name: str
    ) -> Optional[str]:
        """
    ...
    @classmethod
    def get_augmented_ask_slot_utter(
        cls, form_name: str, known_responses: List[str], slot_name: str
    ):
        """
    ...
    def augment_user_turn(
        cls, intent2slots2text, line: str, slot_name2text2value
    ) -> List[Dict[str, Any]]:
        """
        ...
        slots_actual_values = cls._clarify_slots_values(
            slot_name2text2value, slots_dstc2formatted
        )
        (
            slots_to_exclude,
            slots_used_values,
            action_for_text,
        ) = cls._choose_slots_for_whom_exists_text(
            intent2slots2text, slots_actual_values, user_action
        )
        possible_user_response_infos = cls._user_action2text(
            intent2slots2text, action_for_text, slots_used_values
        )
        ...
        for user_response_info in possible_user_response_infos:
            user_utter = {
                ...
                "dialog_acts": [
                    {"act": user_action, "slots": user_response_info["slots"]}
                ],
    ...
    def _choose_slots_for_whom_exists_text(
        ...
        user_action: str,
    ) -> Tuple[List, SLOT2VALUE_PAIRS_TUPLE, str]:
        """
        ...
        for possible_action_key in possible_keys:
            if intent2slots2text[possible_action_key].get(slots_actual_values):
                ...
                if fake_keys:
                    slots_used_values = sorted(
                        fake_keys,
                        key=lambda elem: (
                            len(set(slots_actual_values) ^ set(elem)),
                            len([e for e in elem if e[0] not in slots_lazy_key]),
                        ),
                    )[0]
    ...
    @staticmethod
    def _clarify_slots_values(
        slot_name2text2value: Dict[str, Dict[str, Any]],
        slots_dstc2formatted: List[List],
    ) -> SLOT2VALUE_PAIRS_TUPLE:
        slots_key = []
    ...
    def _parse_user_intent(line: str, ignore_slots=False) -> Tuple[str, List[List]]:
        ...
        if "{" not in intent:
            intent = (
                intent + "{}"
            )  # the prototypical intent is "intent_name{slot1: value1, slotN: valueN}"
        user_action, slots_info = intent.split("{", 1)
        slots_info = json.loads("{" + slots_info)
        slots_dstc2formatted = [
            [slot_name, slot_value] for slot_name, slot_value in slots_info.items()
        ]
        if ignore_slots:
    ...
    def _user_action2text(
        intent2slots2text: Dict[str, Dict[SLOT2VALUE_PAIRS_TUPLE, List]],
        user_action: str,
        slots_li: Optional[SLOT2VALUE_PAIRS_TUPLE] = None,
    ) -> List[str]:
        """
    ...
    @staticmethod
    def _system_action2text(
        domain_knowledge: DomainKnowledge, system_action: str
    ) -> str:
        """
        ...
        possible_system_responses = domain_knowledge.response_templates.get(
            system_action, [{"text": system_action}]
        )

        response_text = possible_system_responses[0]["text"]
        response_text = re.sub(
            r"(\w+)\=\{(.*?)\}", r"#\2", response_text
        )  # TODO: straightforward regex string
