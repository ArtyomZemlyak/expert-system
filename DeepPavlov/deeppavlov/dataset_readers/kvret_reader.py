    ...
    def _check_dialog(dialog):
        ...
        for turn in dialog:
            if turn["turn"] not in ("driver", "assistant"):
                raise RuntimeError(
                    "Dataset wrong format: `turn` key value is"
                    " either `driver` or `assistant`."
                )
            if driver and turn["turn"] != "driver":
                log.debug(
                    "Turn is expected to by driver's, but it's {}'s".format(
                        turn["turn"]
                    )
                )
                return False
            if not driver and turn["turn"] != "assistant":
                log.debug(
                    "Turn is expected to be assistant's but it's {}'s".format(
                        turn["turn"]
                    )
                )
                return False
    ...
    def _iter_file(cls, file_path):
        ...
        for i, sample in enumerate(data):
            ...
            if cls._check_dialog(dialog):
                ...
                log.warning(
                    "Skipping {}th dialogue with uuid={}: wrong format.".format(
                        i, sample["scenario"]["uuid"]
                    )
                )
