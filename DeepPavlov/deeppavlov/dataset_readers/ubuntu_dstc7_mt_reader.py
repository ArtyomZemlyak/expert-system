...
class UbuntuDSTC7MTReader(DatasetReader):
    ...
    def read(
        ...
        **kwargs
        ...
        dataset = {}
        dataset["train"] = self._create_dialog_iter(
            Path(data_path) / "ubuntu_train_subtask_1.json", "train"
        )
        dataset["valid"] = self._create_dialog_iter(
            Path(data_path) / "ubuntu_dev_subtask_1.json", "valid"
        )
        dataset["test"] = self._create_dialog_iter(
            Path(data_path) / "ubuntu_test_subtask_1.json", "test"
        )
        return dataset
        ...
        * label - label of the sample
        ...
        with open(filename, encoding="utf-8") as f:
            ...
            for entry in json_data:
                ...
                if mode != "test":
                    true_response = dialog["options-for-correct-answers"][0][
                        "utterance"
                    ]
                ...
                expanded_context = self._expand_context(
                    utterances, padding=self.padding
                )
                ...
                if mode == "train":
                    ...
                    data.append(
                        (
                            expanded_context
                            + list(self.np_random.choice(fake_responses, size=1)),
                            0,
                        )
                    )  # random 1 from 99
                        ...
                        (
                            ...
                            + list(
                                self.np_random.choice(
                                    fake_responses, self.num_responses - 1
                                )
                            ),
                            0,
                        )
