...
class InsuranceReader(DatasetReader):
    ...
    def read(self, data_path: str, **kwargs) -> Dict[str, List[Tuple[List[str], int]]]:
        ...
        valid_fname = (
            data_path / "insuranceQA-master/V1/question.dev.label.token_idx.pool"
        )
        test_fname = (
            data_path / "insuranceQA-master/V1/question.test1.label.token_idx.pool"
        )
        int2tok_fname = data_path / "insuranceQA-master/V1/vocabulary"
        response2ints_fname = (
            data_path / "insuranceQA-master/V1/answers.label.token_idx"
        )
        self.int2tok_vocab = self._build_int2tok_vocab(int2tok_fname)
        self.idxs2cont_vocab = self._build_context2toks_vocab(
    ...
    def _build_context2toks_vocab(
        self, train_f: Path, val_f: Path, test_f: Path
    ) -> Dict[int, str]:
        contexts = []
    ...
    def _build_int2tok_vocab(self, fname: Path) -> Dict[int, str]:
        with open(fname, "r") as f:
            data = f.readlines()
        int2tok_vocab = {
            int(el.split("\t")[0].split("_")[1]): el.split("\t")[1][:-1] for el in data
        }
    ...
    def _build_response2str_vocab(self, fname: Path) -> Dict[int, str]:
        with open(fname, "r") as f:
            data = f.readlines()
            response2idxs_vocab = {
                int(el.split("\t")[0]) - 1: (el.split("\t")[1][:-1]).split(" ")
                for el in data
            }
        response2str_vocab = {
            el[0]: " ".join([self.int2tok_vocab[int(x.split("_")[1])] for x in el[1]])
    ...
    def _preprocess_data_train(self, fname: Path) -> List[Tuple[List[str], int]]:
        ...
        for k, eli in enumerate(data):
            ...
            q, pa = eli.split("\t")
            q_tok = " ".join(
                [self.int2tok_vocab[int(el.split("_")[1])] for el in q.split()]
            )
