...
class DSTC2DatasetReader(DatasetReader):
    ...
    def _read_from_file(cls, file_path, dialogs=False):
        ...
        utterances, responses, dialog_indices = cls._get_turns(
            cls._iter_file(file_path), with_indices=True
        )
    ...
    def _format_turn(turn):
        turn_x, turn_y = turn
        x = {"text": turn_x["text"], "intents": turn_x["dialog_acts"]}
        if turn_x.get("db_result") is not None:
            x["db_result"] = turn_x["db_result"]
        if turn_x.get("episode_done"):
            x["episode_done"] = True
        y = {"text": turn_y["text"], "act": turn_y["dialog_acts"][0]["act"]}
        return (x, y)
            ...
            if not turn:
                ...
                if speaker == 1:
                    ...
                    if num_dialog_utter - 1 == num_dialog_resp:
                        ...
                        raise RuntimeError(
                            "there cannot be two successive turns of" " speaker 1"
                        )
                    num_dialog_resp += 1
                else:
...
class SimpleDSTC2DatasetReader(DatasetReader):
    ...
    @overrides
    def read(
        self, data_path: str, dialogs: bool = False, encoding="utf-8"
    ) -> Dict[str, List]:
        """
    ...
    def _read_from_file(cls, file_path: str, dialogs: bool = False, encoding="utf-8"):
        ...
        utterances, responses, dialog_indices = cls._get_turns(
            json.load(open(file_path, mode="rt", encoding=encoding)), with_indices=True
        )
    ...
    def _format_turn(turn):
        turn_x, turn_y = turn
        x = {"text": turn_x["text"]}
        y = {"text": turn_y["text"], "act": turn_y["act"]}
        if "act" in turn_x:
            ...
            for i, turn in enumerate(dialog):
                ...
                if speaker == 1:
                    ...
                    if cur_n_utter != cur_n_resp:
                        if i == 0:
                            new_utter = {"text": "", "episode_done": True}
                        else:
                            new_utter = copy.deepcopy(utterances[-1])
                            if "db_result" not in responses[-2]:
                                raise RuntimeError(
                                    "Every api_call action" " should have db_result"
                                )
