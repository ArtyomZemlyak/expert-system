...
def read_infile(
    ...
    *,
    from_words=False,
    word_column: int = WORD_COLUMN,
    pos_column: int = POS_COLUMN,
    tag_column: int = TAG_COLUMN,
    head_column: int = HEAD_COLUMN,
    dep_column: int = DEP_COLUMN,
    max_sents: int = -1,
    read_only_words: bool = False,
    read_syntax: bool = False
) -> List[Tuple[List, Union[List, None]]]:
        ...
        **kwargs
        ...
        if isinstance(data_path, Path):
            ...
            if is_file:
                ...
                if language is None:
                    raise ValueError(
                        "You must implicitly provide language "
        ...
        if has_missing_files and reserve_data_path is not None:
            has_missing_files = any(
                not filepath.exists() for filepath in reserve_data_path
            )
