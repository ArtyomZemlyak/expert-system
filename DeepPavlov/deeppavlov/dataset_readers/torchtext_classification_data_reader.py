...
class TorchtextClassificationDataReader(DatasetReader):
    ...
    def read(
        ...
        valid_portion: Optional[float] = None,
        split_seed: int = 42,
        *args,
        **kwargs,
    ) -> dict:

        if hasattr(torch_texts, dataset_title) and callable(
            getattr(torch_texts, dataset_title)
        ):
            log.info(
                f"Dataset {dataset_title} is used as an attribute of `torchtext.datasets`."
            )
            _text = torchtext.data.RawField()
            _label = torchtext.data.RawField()
            data_splits = getattr(torch_texts, dataset_title).splits(
                _text, _label, root=data_path
            )
            ...
            if "valid" not in splits and valid_portion is not None:
                log.info(
                    "Valid not in `splits` and `valid_portion` is given. Split `train` to `train` and `valid`"
                )
