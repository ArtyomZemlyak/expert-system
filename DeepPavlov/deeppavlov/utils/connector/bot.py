...
from deeppavlov.utils.connector.conversation import (
    AlexaConversation,
    AliceConversation,
    BaseConversation,
)
...
class BaseBot(Thread):
    ...
    def __init__(
        self, model_config: Union[str, Path, dict], input_queue: Queue
    ) -> None:
...
class AlexaBot(BaseBot):
    ...
    def __init__(
        ...
        self._amazon_cert_lifetime = timedelta(
            seconds=bot_config["amazon_cert_lifetime_secs"]
        )
        self._request_timestamp_tolerance_secs = bot_config[
            "request_timestamp_tolerance_secs"
        ]
        self._refresh_valid_certs_period_secs = bot_config[
            "refresh_valid_certs_period_secs"
        ]
    ...
    def _refresh_valid_certs(self) -> None:
        ...
        self._timer = Timer(
            self._refresh_valid_certs_period_secs, self._refresh_valid_certs
        )
        self._timer.start()
    ...
    def _verify_request(
        self, signature_chain_url: str, signature: str, request_body: bytes
    ) -> bool:
        ...
        if signature_chain_url not in self._valid_certificates.keys():
            ...
            if amazon_cert:
                ...
                validated_cert = ValidatedCert(
                    cert=amazon_cert, expiration_timestamp=expiration_timestamp
                )
            ...
            validated_cert: ValidatedCert = self._valid_certificates[
                signature_chain_url
            ]
        ...
        if verify_signature(amazon_cert, signature, request_body):
            ...
            log.error(
                f'Failed signature verification for request: {request_body.decode("utf-8", "replace")}'
            )
            result = False
    ...
    def _handle_request(self, request: dict) -> dict:
        ...
        delta = (
            now - timestamp_datetime
            if now >= timestamp_datetime
            else timestamp_datetime - now
        )

        if abs(delta.seconds) > self._request_timestamp_tolerance_secs:
            log.error(
                f'Failed timestamp check for request: {request_body.decode("utf-8", "replace")}'
            )
        ...
        if conversation_key not in self._conversations:
            self._conversations[conversation_key] = AlexaConversation(
...
class AliceBot(BaseBot):
    ...
    def _handle_request(self, request: dict) -> dict:
        ...
        if conversation_key not in self._conversations:
            self._conversations[conversation_key] = AliceConversation(
                config=self._conversation_config,
                model=self._model,
                self_destruct_callback=self._del_conversation,
                conversation_id=conversation_key,
...
class MSBot(BaseBot):
    ...
    def __init__(
        ...
        connector_config: dict = self._get_connector_params()
        bot_config: dict = connector_config["bot_defaults"]
        bot_config["auth_payload"]["client_id"] = (
            client_id or bot_config["auth_payload"]["client_id"]
        )
        bot_config["auth_payload"]["client_secret"] = (
            client_secret or bot_config["auth_payload"]["client_secret"]
        )
    ...
    def _update_access_info(self) -> None:
        ...
        self._timer = threading.Timer(
            self._auth_polling_interval, self._update_access_info
        )
        self._timer.start()

        result = requests.post(
            url=self._auth_url, headers=self._auth_headers, data=self._auth_payload
        )

        status_code = result.status_code
        if status_code != 200:
            raise HTTPError(
                f"Authentication token request returned wrong HTTP status code: {status_code}"
            )
        ...
        log.info(
            f"Obtained authentication information from Microsoft Bot Framework: {str(access_info)}"
        )
    ...
    def _handle_request(self, request: dict) -> None:
        ...
        if conversation_key not in self._conversations:
            self._conversations[conversation_key] = MSConversation(
                config=self._conversation_config,
                model=self._model,
                self_destruct_callback=self._del_conversation,
                conversation_id=conversation_key,
...
class TelegramBot(BaseBot):
    ...
    def __init__(
        self, model_config: Union[str, Path, dict], token: Optional[str]
    ) -> None:
    ...
    def start(self) -> None:
        ...
        def handle_inference(message: telebot.types.Message) -> None:
            ...
            if chat_id not in self._conversations:
                self._conversations[chat_id] = TelegramConversation(
                    config=self._conversation_config,
                    model=self._model,
                    self_destruct_callback=self._del_conversation,
