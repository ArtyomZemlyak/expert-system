...
class DialogLogger:
    ...
    def __init__(
        self, enabled: bool = False, logger_name: Optional[str] = None
    ) -> None:
    ...
    def _get_log_file(self):
        ...
        """
        log_dir: Path = (
            Path(self.config["log_path"]).expanduser().resolve() / self.logger_name
        )
        log_dir.mkdir(parents=True, exist_ok=True)
        log_file_path = Path(
            log_dir, f"{self._get_timestamp_utc_str()}_{self.logger_name}.log"
        )
        log_file = open(log_file_path, "a", buffering=1, encoding="utf8")
        return log_file

    def _log(
        self, utterance: Any, direction: str, dialog_id: Optional[Hashable] = None
    ):
