...
def verify_sc_url(url: str) -> bool:
    ...
    result = (
        scheme.lower() == "https"
        and netloc.lower().split(":")[0] == "s3.amazonaws.com"
        and path.startswith("/echo.api/")
        and (port == 443 or port is None)
    )
...
def extract_certs(certs_txt: str) -> List[crypto.X509]:
    ...
    certs = [
        crypto.load_certificate(crypto.FILETYPE_PEM, cert_txt) for cert_txt in certs_txt
    ]
    return certs
...
def verify_sans(amazon_cert: crypto.X509) -> bool:
    ...
    """
    cert_extentions = [
        amazon_cert.get_extension(i) for i in range(amazon_cert.get_extension_count())
    ]
    subject_alt_names = ""
...
def verify_certs_chain(
    certs_chain: List[crypto.X509], amazon_cert: crypto.X509
) -> bool:
    ...
    default_verify_file = (
        Path(default_verify_file).resolve() if default_verify_file else None
    )

    default_verify_path = default_verify_paths.capath
    default_verify_path = (
        Path(default_verify_path).resolve() if default_verify_path else None
    )

    ca_files = (
        [ca_file for ca_file in default_verify_path.iterdir()]
        if default_verify_path
        else []
    )
...
def verify_signature(
    amazon_cert: crypto.X509, signature: str, request_body: bytes
) -> bool:
...
def verify_cert(signature_chain_url: str) -> Optional[crypto.X509]:
    ...
    result = (
        sc_url_verification
        and expired_verification
        and sans_verification
        and chain_verification
    )
