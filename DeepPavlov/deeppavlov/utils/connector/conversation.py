...
class BaseConversation:
    ...
    def __init__(
        ...
        logger_name: str = DIALOG_LOGGER_NAME_MAPPING.get(
            type(self).__name__, DIALOG_LOGGER_NAME_MAPPING["_unsupported"]
        )
    ...
    def _act(self, utterance: str) -> str:
        ...
        if len(self._infer_utterances) == len(self._model.in_x):
            ...
            response = self._next_arg_msg.format(
                self._model.in_x[len(self._infer_utterances)]
            )
...
class AlexaConversation(BaseConversation):
    ...
    def __init__(
        ...
        super(AlexaConversation, self).__init__(
            config, model, self_destruct_callback, conversation_id
        )
    ...
    def _generate_response(self, message: str, request: dict) -> dict:
        ...
        response = {
            "version": "1.0",
            "sessionAttributes": {"sessionId": request["session"]["sessionId"]},
            "response": {
                "shouldEndSession": False,
                "outputSpeech": {"type": "PlainText", "text": message},
                "card": {"type": "Simple", "content": message},
            },
        }
...
class AliceConversation(BaseConversation):
    ...
    def _generate_response(self, message: str, request: dict) -> dict:
        ...
        """
        response = {
            "response": {"end_session": False, "text": message},
            "session": {
                "session_id": request["session"]["session_id"],
                "message_id": request["session"]["message_id"],
...
class MSConversation(BaseConversation):
    ...
    def __init__(
        ...
        """
        super(MSConversation, self).__init__(
            config, model, self_destruct_callback, conversation_id
        )
    ...
    def _generate_response(self, message: str, request: dict) -> None:
        ...
        url = urljoin(
            request["serviceUrl"],
            f"v3/conversations/{request['conversation']['id']}/activities",
        )
        ...
        log.debug(
            f"Sent activity to the MSBotFramework server. "
