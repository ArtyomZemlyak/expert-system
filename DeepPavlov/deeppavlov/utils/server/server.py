...
def get_ssl_params(
    ...
    if https:
        ...
        ssl_config = SSLConfig(
            ...
            keyfile=str(ssh_key_path),
            certfile=str(ssh_cert_path),
        )
    else:
...
def redirect_root_to_docs(
    fast_app: FastAPI, func_name: str, endpoint: str, method: str
) -> None:
    ...
    async def redirect_to_docs() -> RedirectResponse:
        operation_id = generate_operation_id_for_path(
            name=func_name, path=endpoint, method=method
        )
...
def start_model_server(
    ...
    ssl_config = get_ssl_params(
        server_params, https, ssl_key=ssl_key, ssl_cert=ssl_cert
    )
    ...
    model_endpoint_post_example = {
        arg_name: ["string"] for arg_name in model_args_names
    }
