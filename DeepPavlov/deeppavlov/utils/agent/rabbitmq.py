...
from deeppavlov.utils.agent.messages import (
    ServiceTaskMessage,
...
class RabbitMQServiceGateway:
    ...
    async def _connect(
        ...
        while True:
            try:
                ...
                reconnect_timeout = 5
                log.error(
                    f"RabbitMQ connection error, making another attempt in {reconnect_timeout} secs"
                )
                time.sleep(reconnect_timeout)

        self._agent_in_channel = await self._connection.channel()
        agent_in_exchange_name = AGENT_IN_EXCHANGE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace
        )
        self._agent_in_exchange = await self._agent_in_channel.declare_exchange(
            name=agent_in_exchange_name, type=aio_pika.ExchangeType.TOPIC
        )
        log.info(f"Declared agent in exchange: {agent_in_exchange_name}")

        self._agent_out_channel = await self._connection.channel()
        agent_out_exchange_name = AGENT_OUT_EXCHANGE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace
        )
        self._agent_out_exchange = await self._agent_in_channel.declare_exchange(
            name=agent_out_exchange_name, type=aio_pika.ExchangeType.TOPIC
        )
    ...
    async def _setup_queues(self, service_name: str, agent_namespace: str) -> None:
        ...
        in_queue_name = SERVICE_QUEUE_NAME_TEMPLATE.format(
            agent_namespace=agent_namespace, service_name=service_name
        )

        self._in_queue = await self._agent_out_channel.declare_queue(
            name=in_queue_name, durable=True
        )
        log.info(f"Declared service in queue: {in_queue_name}")

        service_routing_key = SERVICE_ROUTING_KEY_TEMPLATE.format(
            service_name=service_name
        )
        await self._in_queue.bind(
            exchange=self._agent_out_exchange, routing_key=service_routing_key
        )
        ...
        try:
            ...
            if messages_batch:
                ...
                for message in messages_batch:
                    try:
                        ...
                        log.error(
                            f"Failed to get ServiceTaskMessage from the incoming message: {repr(e)}"
                        )
            ...
            if tasks_batch:
                try:
                    ...
                    log.error(
                        f'got exception {repr(e)} while processing tasks {", ".join(task_ids)}'
                    )
                    formatted_exception = format_exc()
                    error_replies = [
                        self._send_results(task, formatted_exception)
    ...
    async def _process_tasks(self, tasks_batch: List[ServiceTaskMessage]) -> None:
        ...
        task_uuids_batch, payloads = zip(
            *[
                (task.payload["task_id"], task.payload["payload"])
                for task in tasks_batch
            ]
        )
        ...
        responses_batch = await asyncio.wait_for(
            self._interact(payloads), self._utterance_lifetime_sec
        )

        results_replies = [
            self._send_results(task, response)
            for task, response in zip(tasks_batch, responses_batch)
        ]
    ...
    async def _send_results(
        self, task: ServiceTaskMessage, response: Union[Dict, str]
    ) -> None:
        ...
        log.debug(
            f'Sent response for task {str(task.payload["task_id"])} with routing key {routing_key}'
        )
