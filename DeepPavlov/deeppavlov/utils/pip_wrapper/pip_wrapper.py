...
def install(*packages):
    if any(
        _tf_re.match(package) for package in packages
    ) and b"tensorflow-gpu" in subprocess.check_output(
        [sys.executable, "-m", "pip", "freeze"], env=os.environ.copy()
    ):
        log.warning(
            "found tensorflow-gpu installed, so upgrading it instead of tensorflow"
        )
...
def get_config_requirements(config: [str, Path, dict]):
    ...
    requirements |= {
        req for config in config_references for req in get_config_requirements(config)
    }
