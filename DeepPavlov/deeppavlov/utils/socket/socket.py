...
class SocketServer:
    ...
    def __init__(
        ...
        if socket_type == "TCP":
            ...
            self._launch_msg = (
                f'{server_params["socket_launch_message"]} http://{host}:{port}'
            )
    ...
    async def _interact(self, data: dict) -> bytes:
        ...
        model_args += [
            [None] * batch_size for _ in range(len(self._model.in_x) - len(model_args))
        ]
...
def start_socket_server(
    ...
    socket_type: str,
    port: Optional[int],
    socket_file: Optional[Union[str, Path]],
) -> None:
