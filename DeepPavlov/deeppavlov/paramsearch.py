...
parser.add_argument(
    "--search_type", help="search type: grid or random search", type=str, default="grid"
)
    ...
    if args.search_type == "grid":
        ...
        for comb in combinations:
            ...
            if (n_folds is not None) | is_loo:
                ...
                score_dict = calc_cv_score(
                    config, data=data, n_folds=n_folds, is_loo=is_loo
                )
                ...
                if len(data_to_evaluate["valid"]) == 0:
                    (
                        data_to_evaluate["train"],
                        data_to_evaluate["valid"],
                    ) = train_test_split(data_to_evaluate["train"], test_size=0.2)
        ...
        best_params_dict = get_best_params(
            combinations, scores, param_names, target_metric
        )
