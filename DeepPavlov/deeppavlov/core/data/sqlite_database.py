...
class Sqlite3Database(Estimator):
    ...
    def __call__(
        self, batch: List[Dict], order_by: str = None, ascending: bool = False
    ) -> List[List[Dict]]:
    ...
    def _search(self, kv=None, order_by=None, order=""):
        ...
        if kv:
            ...
            self.cursor.execute(
                f"SELECT * FROM {self.tname} WHERE {where_expr}" + order_expr, values
            )
        else:
    ...
    def fit(self, data: List[Dict]) -> None:
        if not self._check_if_table_exists():
            ...
            types = (
                "integer" if isinstance(data[0][k], int) else "text" for k in self.keys
            )
    ...
    def _create_table(self, keys, types):
        ...
        new_types = (
            f"{k} {t} primary key" if k in self.primary_keys else f"{k} {t}"
            for k, t in zip(keys, types)
        )
        new_types_joined = ", ".join(new_types)
        self.cursor.execute(
            f"CREATE TABLE IF NOT EXISTS {self.tname}" f" ({new_types_joined})"
        )
    ...
    def _insert_many(self, data):
        ...
        if to_insert:
            ...
            self.cursor.executemany(
                f"INSERT into {self.tname}" + f" VALUES ({fformat})", to_insert.values()
            )
        if to_update:
    ...
    def _get_record(self, primary_values):
        ...
        fetched = self.cursor.execute(
            f"SELECT {ffields} FROM {self.tname}" + f" WHERE {where_expr}",
            primary_values,
        ).fetchone()
        if not fetched:
            return None
    ...
    def _update_one(self, record):
        ...
        self.cursor.execute(
            f"UPDATE {self.tname}" + f" SET {set_expr}" + f" WHERE {where_expr}",
            set_values + where_values,
        )
