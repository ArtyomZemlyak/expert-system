...
def simple_download(
    url: str, destination: Union[Path, str], headers: Optional[dict] = None
) -> None:
    ...
    if r.status_code != 200:
        raise RuntimeError(
            f"Got status code {r.status_code} when trying to download {url}"
        )
    ...
    if temporary.exists() and temporary.stat().st_size > total_length:
        temporary.write_bytes(
            b""
        )  # clearing temporary file when total_length is inconsistent
    ...
    with temporary.open("ab") as f:
        ...
        with tqdm(
            initial=downloaded, total=total_length, unit="B", unit_scale=True
        ) as pbar:
            while not done:
                if downloaded != 0:
                    log.warning(
                        f"Download stopped abruptly, trying to resume from {downloaded} "
                    ...
                    if (
                        ...
                        or total_length - downloaded != int(r.headers["content-length"])
                    ):
                        raise RuntimeError(
                            f"It looks like the server does not support resuming "
                            f"downloads."
                        )
...
def download(
    dest_file_path: [List[Union[str, Path]]],
    source_url: str,
    ...
    if dest_file_paths:
        ...
        if cache_dir:
            first_dest_path = (
                Path(cache_dir) / md5(source_url.encode("utf8")).hexdigest()[:15]
            )
...
def untar(
    file_path: Union[Path, str], extract_folder: Optional[Union[Path, str]] = None
) -> None:
...
def ungzip(
    file_path: Union[Path, str], extract_path: Optional[Union[Path, str]] = None
) -> None:
...
def download_decompress(
    ...
    if not extracted:
        ...
        if file_name.endswith(".tar.gz"):
            ...
            ungzip(
                arch_file_path, extracted_path / Path(file_name).with_suffix("").name
            )
...
def _get_all_dimensions(
    batch: Sequence, level: int = 0, res: Optional[List[List[int]]] = None
) -> List[List[int]]:
...
def zero_pad_truncate(
    ...
    """
    if isinstance(
        batch[0][0], Collection
    ):  # ndarray behaves like a Sequence without actually being one
    ...
    for i, batch_item in enumerate(batch):
        if len(batch_item) > max_len:  # trunc
            padded_batch[i] = batch_item[
                slice(max_len) if trunc == "post" else slice(-max_len, None)
            ]
        else:  # pad
            padded_batch[
                i,
                slice(len(batch_item))
                if pad == "post"
                else slice(-len(batch_item), None),
            ] = batch_item
