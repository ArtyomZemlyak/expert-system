...
def bi_rnn(
    ...
    with tf.variable_scope(name + "_" + cell_type.upper()):
        if cell_type == "gru":
            forward_cell = tf.nn.rnn_cell.GRUCell(
                n_hidden, kernel_initializer=INITIALIZER()
            )
            backward_cell = tf.nn.rnn_cell.GRUCell(
                n_hidden, kernel_initializer=INITIALIZER()
            )
            if trainable_initial_states:
                initial_state_fw = tf.tile(
                    tf.get_variable("init_fw_h", [1, n_hidden]), (tf.shape(units)[0], 1)
                )
                initial_state_bw = tf.tile(
                    tf.get_variable("init_bw_h", [1, n_hidden]), (tf.shape(units)[0], 1)
                )
            else:
                initial_state_fw = initial_state_bw = None
        elif cell_type == "lstm":
            forward_cell = tf.nn.rnn_cell.LSTMCell(
                n_hidden, use_peepholes=use_peepholes, initializer=INITIALIZER()
            )
            ...
            if trainable_initial_states:
                initial_state_fw = tf.nn.rnn_cell.LSTMStateTuple(
                    tf.tile(
                        tf.get_variable("init_fw_c", [1, n_hidden]),
                        (tf.shape(units)[0], 1),
                    ),
                    tf.tile(
                        tf.get_variable("init_fw_h", [1, n_hidden]),
                        (tf.shape(units)[0], 1),
                    ),
                )
                initial_state_bw = tf.nn.rnn_cell.LSTMStateTuple(
                    tf.tile(
                        tf.get_variable("init_bw_c", [1, n_hidden]),
                        (tf.shape(units)[0], 1),
                    ),
                    tf.tile(
                        tf.get_variable("init_bw_h", [1, n_hidden]),
                        (tf.shape(units)[0], 1),
                    ),
                )
            else:
        ...
        (rnn_output_fw, rnn_output_bw), (fw, bw) = tf.nn.bidirectional_dynamic_rnn(
            forward_cell,
            backward_cell,
            units,
            dtype=tf.float32,
    ...
    kernels = [
        var
        for var in forward_cell.trainable_variables + backward_cell.trainable_variables
    ...
    for n, n_hidden in enumerate(n_hidden_list):
        with tf.variable_scope(name + "_" + str(n)):
            if cell_type == "gru":
                ...
                forward_cell = tf.nn.rnn_cell.LSTMCell(
                    n_hidden, use_peepholes=use_peepholes
                )
                backward_cell = tf.nn.rnn_cell.LSTMCell(
                    n_hidden, use_peepholes=use_peepholes
                )
...
def u_shape(
    ...
    for down_step, n_hidden in enumerate(n_hidden_list[::-1]):
        ...
        units = tf.layers.conv2d_transpose(
            units, n_hidden, filter_width, strides=(2, 1), padding="same"
        )
...
def stacked_highway_cnn(
    ...
    for n_layer, n_hidden in enumerate(n_hidden_list):
        ...
        if use_batch_norm:
            units = tf.layers.batch_normalization(units, training=training_ph)
        sigmoid_gate = tf.layers.dense(
            input_units, 1, activation=tf.sigmoid, kernel_initializer=INITIALIZER()
        )
...
def embedding_layer(
    ...
    if token_embedding_matrix is not None:
        ...
        tok_mat = np.random.randn(n_tokens, token_embedding_dim).astype(
            np.float32
        ) / np.sqrt(token_embedding_dim)
...
def character_embedding_network(
    char_placeholder: tf.Tensor,
    ...
    """
    if emb_mat is None:
        emb_mat = np.random.randn(n_characters, char_embedding_dim).astype(
            np.float32
        ) / np.sqrt(char_embedding_dim)
    else:
...
def additive_self_attention(
    units, n_hidden=None, n_output_features=None, activation=None
):
    ...
    query = tf.layers.dense(
        units_pairs, n_hidden, activation=tf.tanh, kernel_initializer=INITIALIZER()
    )
    ...
    output = tf.layers.dense(
        attended_units, n_output_features, activation, kernel_initializer=INITIALIZER()
    )
    return output


def multiplicative_self_attention(
    units, n_hidden=None, n_output_features=None, activation=None
):
    ...
    queries = tf.layers.dense(
        expand_tile(units, 1), n_hidden, kernel_initializer=INITIALIZER()
    )
    keys = tf.layers.dense(
        expand_tile(units, 2), n_hidden, kernel_initializer=INITIALIZER()
    )
...
def cudnn_lstm(
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        h, (h_last, c_last) = lstm(
            tf.transpose(units, (1, 0, 2)), (initial_h, initial_c)
        )
        h = tf.transpose(h, (1, 0, 2))
        h_last = h_last[-1]
        c_last = c_last[-1]
...
def cudnn_compatible_lstm(
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope("cudnn_lstm", reuse=reuse):
            ...
            init = tuple(
                [
                    tf.nn.rnn_cell.LSTMStateTuple(ic, ih)
                    for ih, ic in zip(
                        tf.unstack(initial_h, axis=0), tf.unstack(initial_c, axis=0)
                    )
                ]
            )

            h, state = tf.nn.dynamic_rnn(
                cell=cell, inputs=units, time_major=True, initial_state=init
            )
...
def cudnn_bi_gru(
    ...
    with tf.variable_scope(name, reuse=reuse):
        if seq_lengths is None:
            seq_lengths = (
                tf.ones([tf.shape(units)[0]], dtype=tf.int32) * tf.shape(units)[1]
            )
        ...
        with tf.variable_scope("Backward"):
            reversed_units = tf.reverse_sequence(
                units, seq_lengths=seq_lengths, seq_dim=1, batch_dim=0
            )
            ...
            h_bw = tf.reverse_sequence(
                h_bw, seq_lengths=seq_lengths, seq_dim=1, batch_dim=0
            )
...
def cudnn_bi_lstm(
    ...
    with tf.variable_scope(name, reuse=reuse):
        ...
        with tf.variable_scope("Forward"):
            h_fw, (h_fw_last, c_fw_last) = cudnn_lstm_wrapper(
                units,
                n_hidden,
                n_layers=n_layers,
                trainable_initial_states=trainable_initial_states,
        ...
        with tf.variable_scope("Backward"):
            ...
            h_bw, (h_bw_last, c_bw_last) = cudnn_lstm_wrapper(
                reversed_units,
                n_hidden,
                n_layers=n_layers,
                trainable_initial_states=trainable_initial_states,
