...
def general_attention(key, context, hidden_size, projected_align=False):
    ...
    (output_fw, output_bw), states = tf.nn.bidirectional_dynamic_rnn(
        cell_fw=lstm_fw_cell, cell_bw=lstm_bw_cell, inputs=r_context, dtype=tf.float32
    )
    ...
    if projected_align:
        ...
        output = tf.reshape(
            tf.matmul(t_context, attn), shape=[batch_size, -1, hidden_size]
        )
    else:
        log.info("Using without projected attention alignment")
        t_context = tf.transpose(r_context, [0, 2, 1])
        output = tf.reshape(
            tf.matmul(t_context, attn), shape=[batch_size, -1, token_size]
        )
    return output
...
def light_general_attention(key, context, hidden_size, projected_align=False):
    ...
    projected_context = tf.layers.dense(
        r_context, hidden_size, kernel_initializer=xav()
    )
...
def cs_general_attention(key, context, hidden_size, depth, projected_align=False):
    ...
    projected_context = tf.layers.dense(
        r_context, token_size, kernel_initializer=xav(), name="projected_context"
    )
    ...
    if projected_align:
        ...
        output = tf.reshape(
            aligned_h_state, shape=[batch_size, -1, depth * hidden_size]
        )
    else:
        log.info("Using without projected attention alignment")
...
def bahdanau_attention(key, context, hidden_size, projected_align=False):
    ...
    # projected_key: [None, None, hidden_size]
    projected_key = tf.layers.dense(key, hidden_size, kernel_initializer=xav())
    r_projected_key = tf.tile(
        tf.reshape(projected_key, shape=[-1, 1, hidden_size]), [1, max_num_tokens, 1]
    )
    ...
    projected_state = tf.layers.dense(
        concat_h_state, hidden_size, use_bias=False, kernel_initializer=xav()
    )
    score = tf.layers.dense(
        tf.tanh(projected_state), units=1, use_bias=False, kernel_initializer=xav()
    )
...
def cs_bahdanau_attention(key, context, hidden_size, depth, projected_align=False):
    ...
    # projected_key: [None, None, hidden_size]
    projected_key = tf.layers.dense(
        key, hidden_size, kernel_initializer=xav(), name="projected_key"
    )
    r_projected_key = tf.tile(
        tf.reshape(projected_key, shape=[-1, 1, hidden_size]), [1, max_num_tokens, 1]
    )
