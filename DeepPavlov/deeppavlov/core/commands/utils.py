...
def _parse_config_property(
    item: _T,
    variables: Dict[str, Union[str, Path, float, bool, int, None]],
    variables_exact: Dict[str, Union[str, Path, float, bool, int, None]],
) -> _T:
    ...
    if isinstance(item, str):
        ...
        return [
            _parse_config_property(item, variables, variables_exact) for item in item
        ]
    elif isinstance(item, dict):
        return {
            k: _parse_config_property(v, variables, variables_exact)
...
def _get_variables_from_config(config: Union[str, Path, dict]):
    ...
    variables = {
        "DEEPPAVLOV_PATH": os.getenv(
            f"DP_DEEPPAVLOV_PATH", Path(__file__).parent.parent.parent
        )
    }
...
def _update_requirements(config: dict) -> dict:
    ...
    components = {
        inverted_registry.get(component, component) for component in components
    }
    requirements_registry_path = (
        Path(__file__).parents[1] / "common" / "requirements_registry.json"
    )
...
def parse_value_with_config(
    value: Union[str, Path], config: Union[str, Path, dict]
) -> Path:
