...
from deeppavlov.core.trainers.utils import (
    Metric,
    parse_metrics,
    prettify_metrics,
    NumpyArrayEncoder,
...
class FitTrainer:
        ...
        **kwargs,
        ...
        if tensorboard_log_dir is not None:
            try:
                ...
                log.warning(
                    "TensorFlow could not be imported, so tensorboard log directory"
    ...
    def fit_chainer(
        self, iterator: Union[DataFittingIterator, DataLearningIterator]
    ) -> None:
        """
        ...
        for component_index, component_config in enumerate(
            self.chainer_config["pipe"], 1
        ):
            ...
            if "fit_on" in component_config:
                ...
                if self.batch_size > 0 and callable(
                    getattr(component, "partial_fit", None)
                ):
                    writer = None

                    for i, (x, y) in enumerate(
                        iterator.gen_batches(self.batch_size, shuffle=False)
                    ):
                        ...
                        if result is not None and self.tensorboard_log_dir is not None:
                            if writer is None:
                                writer = self._tf.summary.FileWriter(
                                    str(
                    ...
                    result: Optional[Dict[str, Iterable[float]]] = component.fit(
                        *preprocessed
                    )

                    if result is not None and self.tensorboard_log_dir is not None:
                        writer = self._tf.summary.FileWriter(
                            str(self.tensorboard_log_dir / f"fit_log_{component_index}")
                        )
    ...
    def test(
        ...
        *,
        ...
        expected_outputs = list(
            set().union(self._chainer.out_params, *[m.inputs for m in metrics])
        )
        ...
        for x, y_true in data:
            examples += len(x)
            y_predicted = list(
                self._chainer.compute(list(x), list(y_true), targets=expected_outputs)
            )
        ...
        if examples == 0:
            ...
            return {
                ...
                "metrics": None,
                "time_spent": str(datetime.timedelta(seconds=0)),
            }
        ...
        if show_examples:
            ...
            report["examples"] = [
                {"x": x_item, "y_predicted": y_predicted_item, "y_true": y_true_item}
        ...
        *,
        ...
        for data_type in evaluation_targets:
            ...
            if print_reports:
                print(
                    json.dumps(
                        {data_type: report}, ensure_ascii=False, cls=NumpyArrayEncoder
                    )
                )
