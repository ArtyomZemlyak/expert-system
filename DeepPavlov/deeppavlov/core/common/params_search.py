...
class ParamsSearch:
    ...
    def __init__(self, prefix="search", seed: int = None, **kwargs):
        """
    ...
    def find_model_path(
        self, config: dict, key_model: str, path: list = []
    ) -> Generator:
        """
        ...
        if isinstance(config_pointer, dict) and key_model in config_pointer.keys():
            ...
            if isinstance(config_pointer, dict):
                for key in list(config_pointer.keys()):
                    for path_ in self.find_model_path(
                        config_pointer[key], key_model, path + [key]
                    ):
                        yield path_
            elif isinstance(config_pointer, list):
                for i in range(len(config_pointer)):
                    for path_ in self.find_model_path(
                        config_pointer[i], key_model, path + [i]
                    ):
                        yield path_

    @staticmethod
    def insert_value_or_dict_into_config(
        config: dict, path: list, value: [int, float, str, bool, list, dict, np.ndarray]
    ) -> None:
        """
    ...
    def initialize_params_in_config(
        self, basic_config: dict, paths: List[list]
    ) -> dict:
        """
    ...
    def sample_params(self, **params) -> dict:
        ...
        for param, param_val in params_copy.items():
            if isinstance(param_val, dict):
                if (
                    self.prefix + "_bool" in param_val
                    and param_val[self.prefix + "_bool"]
                ):
    ...
    @staticmethod
    def _sample_log(from_: float = 0.0, to_: float = 1.0) -> float:
        """
