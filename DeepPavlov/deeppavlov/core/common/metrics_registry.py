...
def fn_from_str(name: str) -> Callable[..., Any]:
    ...
    try:
        ...
        raise ConfigError(
            "Expected function description in a `module.submodules:function_name` form, but got `{}`".format(
                name
            )
        )
...
def register_metric(metric_name: str) -> Callable[..., Any]:
    ...
    def decorate(fn):
        ...
        if metric_name in _REGISTRY and _REGISTRY[metric_name] != fn_name:
            log.warning(
                '"{}" is already registered as a metric name, the old function will be ignored'.format(
                    metric_name
                )
            )
