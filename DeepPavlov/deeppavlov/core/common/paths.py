...
_settings_path = (
    Path(os.getenv("DP_SETTINGS_PATH", _default_settings_path)).expanduser().resolve()
)
if _settings_path.is_file():
    raise FileExistsError(
        f"DP_SETTINGS_PATH={_settings_path} is a file and not a directory"
    )

if _default_settings_path in _settings_path.parents:
    raise RecursionError(
        f"DP_SETTINGS_PATH={_settings_path} is relative"
