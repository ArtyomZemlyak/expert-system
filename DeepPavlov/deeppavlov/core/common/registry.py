...
def cls_from_str(name: str) -> type:
    ...
    try:
        ...
        raise ConfigError(
            "Expected class description in a `module.submodules:ClassName` form, but got `{}`".format(
                name
            )
        )
...
def register(name: str = None) -> type:
    ...
    def decorate(model_cls: type, reg_name: str = None) -> type:
        ...
        if model_name in _REGISTRY and _REGISTRY[model_name] != cls_name:
            logger.warning(
                'Registry name "{}" has been already registered and will be overwritten.'.format(
                    model_name
                )
            )
