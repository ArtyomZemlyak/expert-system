...
class Chainer(Component):
    ...
    def __init__(
        self,
        in_x: Union[str, list] = None,
        out_params: Union[str, list] = None,
        in_y: Union[str, list] = None,
        *args,
        **kwargs,
    ) -> None:
    ...
    def __repr__(self):
        ...
        for in_params, out_params, component in self.train_pipe:
            ...
            if (
                isinstance(component, Hashable)
                and component in reversed_components_dict
            ):
    ...
    def append(
        ...
        if in_y is not None:
            ...
            component: NNModel
            main = True
            assert self.train_map.issuperset(
                in_x + in_y
    ...
    def __call__(self, *args):
        return self._compute(
            *args, param_names=self.in_x, pipe=self.pipe, targets=self.out_params
        )
    ...
    def _compute(*args, param_names, pipe, targets):
        ...
        if not expected.issubset(param_names):
            raise RuntimeError(
                f"{expected} are required to compute {targets} but were not found in memory or inputs"
            )
        pipe = final_pipe
    ...
    def batched_call(
        self, *args: Reversible, batch_size: int = 16
    ...
    def serialize(self) -> bytes:
        ...
        for in_params, out_params, component in self.train_pipe:
            serialized = (
                component.serialize() if isinstance(component, Component) else None
            )
    ...
    def deserialize(self, data: bytes) -> None:
        ...
        for (in_params, out_params, component), component_data in zip(
            self.train_pipe, data
        ):
