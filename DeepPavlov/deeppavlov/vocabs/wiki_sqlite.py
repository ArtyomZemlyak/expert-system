...
class WikiSQLiteVocab(SQLiteDataIterator, Component):
    ...
    def __init__(
        self, load_path: str, join_docs: bool = True, shuffle: bool = False, **kwargs
    ) -> None:
        SQLiteDataIterator.__init__(self, load_path=load_path, shuffle=shuffle)
        self.join_docs = join_docs

    def __call__(
        self, doc_ids: Optional[List[List[Any]]] = None, *args, **kwargs
        ...
        if not doc_ids:
            logger.warning(
                "No doc_ids are provided in WikiSqliteVocab, return all docs"
            )
