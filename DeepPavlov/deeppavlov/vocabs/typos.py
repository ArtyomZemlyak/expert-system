...
class RussianWordsVocab(StaticDictionary):
    ...
    def _get_source(*args, **kwargs):
        log.info(
            "Downloading russian vocab from https://github.com/danakt/russian-words/"
        )
        url = "https://github.com/danakt/russian-words/raw/master/russian.txt"
        page = requests.get(url)
        return [
            word.strip() for word in page.content.decode("cp1251").strip().split("\n")
        ]
...
class Wiki100KDictionary(StaticDictionary):
    ...
    def _get_source(*args, **kwargs):
        ...
        for i in range(1, 100000, 10000):
            k = 10000 + i - 1
            url = "https://en.wiktionary.org/wiki/Wiktionary:Frequency_lists/PG/2005/08/{}-{}".format(
                i, k
            )
            page = requests.get(url)
