...
class MultiSquadIterator(DataLearningIterator):
    ...
    def gen_batches(
        ...
        Tuple[Tuple[Tuple[str, str]], Tuple[List[str], List[int]]], None, None
    ]:
        ...
        for i in range((data_len - 1) // batch_size + 1):
            ...
            for j in range(i * batch_size, min((i + 1) * batch_size, data_len)):
                ...
                if random.rand() < self.with_answer_rate or len(noans_contexts) == 0:
                    ...
                    context = noans_contexts[
                        np.argmax(random.multinomial(1, noans_scores))
                    ]

                answer_text = (
                    [ans["text"] for ans in context["answer"]]
                    if len(context["answer"]) > 0
                    else [""]
                )
                answer_start = (
                    [ans["answer_start"] for ans in context["answer"]]
                    if len(context["answer"]) > 0
                    else [-1]
                )
    ...
    def get_instances(
        self, data_type: str = "train"
    ) -> Tuple[Tuple[Tuple[str, str]], Tuple[List[str], List[int]]]:
        data_examples = []
        for qcas in self.data[data_type]:  # question, contexts, answers
            ...
            for context in qcas["contexts"]:
                ...
                data_examples.append(
                    ((context["context"], question), (answer_text, answer_start))
                )
...
class MultiSquadRetrIterator(DataLearningIterator):
    ...
    def __init__(
        ...
        squad_rate: Optional[float] = None,
        *args,
        **kwargs
    ) -> None:
        self.with_answer_rate = with_answer_rate
                ...
                for i in range(batch_size):
                    ...
                    if with_answer_rate is None:
                        with_answer_rate = (
                            1.0
                            if noans_clen == 0
                            else ans_clen / (ans_clen + noans_clen)
                        )
                    ...
                    if random.rand() < with_answer_rate or noans_clen == 0:
                        ...
                        if self.squad_rate is not None:
                            if (
                                random.rand() < self.squad_rate
                                or len(ans_contexts) == 1
                            ):
