...
class SQLiteDataIterator(DataFittingIterator):
    ...
    def map_doc2idx(self) -> Dict[int, Any]:
        ...
        logger.info(
            "SQLite iterator: The size of the database is {} documents".format(
                len(doc2idx)
            )
        )
        return doc2idx
    ...
    @overrides
    def gen_batches(
        self, batch_size: int, shuffle: bool = None
        ...
        if batch_size > 0:
            batches = [
                _doc_ids[i : i + batch_size]
                for i in range(0, len(_doc_ids), batch_size)
            ]
        else:
            batches = [_doc_ids]
