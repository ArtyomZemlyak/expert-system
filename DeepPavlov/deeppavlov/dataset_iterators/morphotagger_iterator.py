...
def preprocess_data(
    ...
    for words, tags in data:
        new_words = [
            process_word(word, to_lower=to_lower, append_case=append_case)
            for word in words
        ]
...
class MorphoTaggerDatasetIterator(DataLearningIterator):
    ...
    def split(self, *args, **kwargs) -> None:
        ...
        if len(self.valid) == 0:
            ...
            gap = (
                int(self.min_train_fraction * (train_length + valid_length))
                - train_length
            )
            if gap > 0:
    ...
    def gen_batches(
        ...
        for start in starts:
            ...
            data_to_yield = tuple(
                list(x) for x in zip(*([data[i] for i in indexes_to_yield]))
            )
            if return_indexes:
