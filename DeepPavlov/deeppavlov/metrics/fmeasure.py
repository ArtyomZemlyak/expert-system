...
def ner_f1(y_true, y_predicted):
    ...
    results = precision_recall_f1(y_true, y_predicted, print_results=True)
...
def _print_conll_report(
    ...
    if not short_report:
        for tag in tags:
            if entity_of_interest is not None:
                if entity_of_interest in tag:
                    s += (
                        "\t" + tag + ": precision:  {tot_prec:.2f}%; "
                        "recall:  {tot_recall:.2f}%; "
                        "F1:  {tot_f1:.2f} "
                        "{tot_predicted}\n\n".format(
                            tot_prec=results[tag]["precision"],
        ...
        s += (
            "\t" + entity_of_interest + ": precision:  {tot_prec:.2f}%; "
            "recall:  {tot_recall:.2f}%; "
            "F1:  {tot_f1:.2f} "
            "{tot_predicted}\n\n".format(
...
def chunk_finder(current_token, previous_token, tag):
    ...
    if current_tag != "O" and (
        previous_tag == "O"
        or previous_token in ["E-" + tag, "L-" + tag, "S-" + tag, "U-" + tag]
        or current_token in ["B-" + tag, "S-" + tag, "U-" + tag]
    ):
        create_chunk = True
    else:
        create_chunk = False
...
def precision_recall_f1(
    y_true, y_pred, print_results=True, short_report=False, entity_of_interest=None
):
    # Find all tags
    tags = set()
...
def roc_auc__roc_auc__ner_f1(
    true_onehot1, pred_probas1, true_onehot2, pred_probas2, ner_true3, ner_pred3
):
