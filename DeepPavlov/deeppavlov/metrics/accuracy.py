...
def per_item_action_accuracy(
    dialogs_true, dialog_jsons_predicted: List[List[JSONNLGResponse]]
):
    # todo metric classes???
    # todo oop instead of serialization/deserialization
    utterances_actions_true = [
        utterance["act"] for dialog in dialogs_true for utterance in dialog
    ]

    utterances_actions_predicted: Iterable[JSONNLGResponse] = itertools.chain(
        *dialog_jsons_predicted
    )
    examples_len = len(utterances_actions_true)
    correct = sum(
        [
            y1.strip().lower() == "+".join(y2.actions_tuple).lower()
            for y1, y2 in zip(utterances_actions_true, utterances_actions_predicted)
        ]
    )  # todo ugly
    return correct / examples_len if examples_len else 0
