...
class StateSkill(metaclass=DSLMeta):
    ...
    @DSLMeta.handler(commands=["bye"], state="state1")
    def bye(context):
        response = "bb!"
        confidence = 1.0
        return response, confidence
...
class ContextConditionSkill(metaclass=DSLMeta):
    @DSLMeta.handler(
        commands=["hello", "hi", "sup", "greetings"],
...
class TestDSLSkill:
    ...
    def test_simple_skill(self):
        user_messages_sequence = ["Hello", "Hi", "Tell me a joke", "Sup", "Ok, goodbye"]
    ...
    def test_switch_state(self):
        user_messages_sequence = ["Hello", "bye", "bye"]
    ...
    def test_context_condition(self):
        user_messages_sequence = ["Hello", "Hi"]
