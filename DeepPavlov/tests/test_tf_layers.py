...
from deeppavlov.core.layers.tf_layers import (
    cudnn_lstm,
...
class DPCudnnLSTMModel:
    def __init__(self, num_layers, num_units):
        ...
        with tf.variable_scope("cudnn_model"):
            h, (h_last, c_last) = cudnn_lstm(
                self.x, num_units, num_layers, trainable_initial_states=True
            )
...
class DPLSTMModel:
    def __init__(self, num_layers, num_units):
        ...
        self.x = tf.placeholder(shape=(None, None, 50), dtype=tf.float32)
        with tf.variable_scope("cudnn_model"):
            h, (h_last, c_last) = cudnn_compatible_lstm(
                self.x, num_units, num_layers, trainable_initial_states=True
            )
