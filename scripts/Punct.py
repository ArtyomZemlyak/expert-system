...
def compute_performance(config, model, loader):
    ...
    for x, y in loader:
        ...
        with torch.no_grad():
            ...
            loss1 = criterion(
                y_scores1.view(y1.size(0) * y1.size(1), -1),
                y1.view(y1.size(0) * y1.size(1)),
            )
            loss2 = criterion(
                y_scores2.view(y2.size(0) * y2.size(1), -1),
                y2.view(y2.size(0) * y2.size(1)),
            )
            loss = loss1 + loss2
    ...
    for label in range(0, 5):
        ...
        precision[label] = (
            num_correct[label] / num_hyp[label] if num_hyp[label] > 0 else 0
        )
        fscore[label] = (
            (
                2
            ...
            if recall[label] + precision[label] > 0
            else 0
        )
    return (
        total_loss / num_loss,
        all_correct2.item() / num_perf,
        all_correct1.item() / num_perf,
        fscore,
    )
        ...
        for x, y in tqdm(train_loader):
            ...
            if iteration % valid_period == valid_period - 1:
                ...
                (
                    ...
                    valid_accuracy_punc,
                    valid_fscore,
                ) = compute_performance(config, model, valid_loader)
                torch.save(
                    {
...
def train(config, train_x_fn, train_y_fn, valid_x_fn, valid_y_fn, checkpoint_path):
    X_train, Y_train = batchify(
        config.max_length, torch.load(train_x_fn), torch.load(train_y_fn)
    )
    X_valid, Y_valid = batchify(
        config.max_length, torch.load(valid_x_fn), torch.load(valid_y_fn)
    )
...
def run_eval(config, test_x_fn, test_y_fn, checkpoint_path):
    X_test, Y_test = batchify(
        config.max_length, torch.load(test_x_fn), torch.load(test_y_fn)
    )
...
class CasePuncPredictor:
    def __init__(
        ...
        tokens = list(
            enumerate(
                self.tokenize(
                    "тестовое предложение для разогрева тут будут всякие разные слова, да побольше чтобы лучше разогреть нейронку довольн оинтереснный эффетк зачем это нуно intresno a english slova on smozhet na defolte"
                )
            )
        )
        for _ in self.predict(tokens, lambda x: x[1]):
            pass
...
def generate_predictions(config, checkpoint_path):
    ...
    for line in sys.stdin:
        ...
        if config.debug:
            print(line)
        tokens = (
            [config.cls_token] + config.tokenizer.tokenize(line) + [config.sep_token]
        )
        if config.debug:
            print(tokens)
        previous_label = punctuation["PERIOD"]
        ...
        for start in range(0, len(tokens), config.max_length):
            ...
            y_pred1 = torch.max(y_scores1, 2)[1]
            y_pred2 = torch.max(y_scores2, 2)[1]
            for id, token, punc_label, case_label in zip(
                ids,
                instance,
                y_pred1[0].tolist()[: len(instance)],
                y_pred2[0].tolist()[: len(instance)],
        ...
        for token in text.strip().split():
            ...
            while start < len(chars):
                ...
                while start < end:
                    ...
                    if (self.keep_case and substr.lower() in self.vocab) or (
...
def bpe(self, token):
    ...
    while True:
        bigram = min(
            pairs, key=lambda pair: self.bpe_ranks.get(to_lower(pair), float("inf"))
        )
        # print(bigram)
...
def init(config):
    ...
    if config.lang == "fr":
        config.tokenizer = tokenizer = AutoTokenizer.from_pretrained(
            config.flavor, do_lower_case=False
        )
        ...
        config.tokenizer = tokenizer = BertTokenizer.from_pretrained(
            config.flavor, do_lower_case=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "action", help="train|eval|predict|tensorize|preprocess", type=str
    )
    parser.add_argument(
        "action_args", help="arguments for selected action", type=str, nargs="*"
    )
    parser.add_argument(
        "--seed", help="random seed", default=default_config.seed, type=int
    )
    parser.add_argument(
        "--lang", help="language (fr, en, zh)", default=default_config.lang, type=str
    )
    parser.add_argument(
        "--flavor",
        help="bert flavor in transformers model zoo",
        default=default_config.flavor,
        type=str,
    )
    parser.add_argument(
        "--max-length",
    ...
    parser.add_argument(
        "--debug",
        help="whether to output more debug info",
    ...
    parser.add_argument(
        ...
        default=default_config.updates,
        type=bool,
    )
    parser.add_argument(
        "--period",
        help="validation period in updates",
        default=default_config.period,
        type=bool,
    )
    parser.add_argument(
        "--lr", help="learning rate", default=default_config.lr, type=bool
    )
    parser.add_argument(
        "--dab-rate",
        help="drop at boundaries rate",
        default=default_config.dab_rate,
        type=bool,
    )
